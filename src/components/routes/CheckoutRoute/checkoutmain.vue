<template>
  <div class="main">
    ...
    <section id="details" class="lg:w-2/3 mb-10">
      <div>
        <div class="font-bold text-white text-xl mb-6 relative flex items-center justify-between cursor-pointer group">
          <span>邮箱地址</span>
          ...
          <div class="popup absolute top-0 right-0 mt-8 text-gray-500 bg-black-80 rounded-lg text-sm font-normal max-w-md py-1 px-2 text-right pointer-events-none opacity-0 transition-opacity duration-150 ease-in-out group-hover:opacity-100">
            您的电子邮件将仅用于发送付款确认和其他重要信息。 你永远都不会由 Arcane Travel 发送垃圾邮件或发送垃圾邮件。
          </div>
        </div>
        <div class="form-row" @click.once="enableCheck">
          <label for="email" class="sr-only">邮箱地址</label>
          <input name="email" id="email" v-model="Email" ref="email" type="email" :class="ifIsEmail" placeholder="example@arcanetravel.com" class="appearance-none bg-gray-800 border border-lighten font-bold text-lg block w-full py-3 px-5 transition-colors duration-150 ease-in-out focus:border-yellow-400 focus:outline-none focus:border-piston">
        </div>
      </div>
    </section>
...
function check(value) {
  ...
  if (value === "") { //输入不能为空
    ...
    GLOBAL_DATA.state.fields.email_add = value
    console.log(GLOBAL_DATA.state.fields.email_add)
    ifIsEmail.value = ''
    return true;
  }
}
